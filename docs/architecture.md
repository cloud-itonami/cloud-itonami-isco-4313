...
The distinction in the last row is load-bearing. Naming an article as unread
...
| 3 | `kotoba-lang/taxlaw` | 別表第二 / 別表第五 (源泉徴収税額表) are recorded as unread. Until they are read, every 所得税 figure here is `:declared`. |
...
| 6 | this repository, next phase | 住民税 特別徴収 (地方税法 第三百二十一条の五, unread) — no rule, no line, no account. The MoneyForward boundary reports it as a named gap and blocks reconciliation when the column carries a value. |
