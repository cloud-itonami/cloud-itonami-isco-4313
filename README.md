...
Thirteen mutations cover the route (`nbb tools/mutate.kotoba`); the table's
...
`test/payroll/conformance_test.kotoba` pins every disposition against
...
That objection is answered by there being no subset. `tools/import_nta_2026.kotoba`
**reads** the 国税庁 workbook and writes `src/payroll/rates/monthly_2026.kotoba`:
...
and `test/payroll/store_contract_test.kotoba` runs **every** assertion against both
...
`src/payroll/edge/endpoints.kotoba`, portable `.cljc`, `{:status n :body {...}}`
in and out, no host effects and no framework. `payroll.host.jvm` mounts it,
and `src/payroll/edge/console.kotoba` adds the operator screens on the same
